...
struct Line3D {
    ...
    Line3D(float x = 0, float y = 0, float z = 0, float mx = 0, float my = 0, float mz = 0) : x(x), y(y), z(z), mx(mx),
                                                                                              my(my), mz(mz) {}
...
struct HomogeneousPoint3D {
    ...
    float
    magnitude() { //TODO: Think about this. Should this be w or abs(w). Does w capture angle better in asin(wedge(l1,l2))?
        return w;
    }
...
struct Motor3D {
    ...
    Motor3D(float s = 1, float wx = 0, float wy = 0, float wz = 0, float yz = 0, float zx = 0, float xy = 0,
            float wxyz = 0) : s(s), wx(wx), wy(wy), wz(wz), yz(yz), zx(zx), xy(xy), wxyz(wxyz) {}

    Motor3D(float s, Line3D l) : s(s), wx(l.mx), wy(l.my), wz(l.mz), yz(l.x), zx(l.y), xy(l.z) {}

    Motor3D(MultiVector mv) : s(mv.s), wx(mv.wx), wy(mv.wy), wz(mv.wz), yz(mv.yz), zx(mv.zx), xy(mv.xy),
                              wxyz(mv.wxyz) {};
    ...
    //TODO: maybe a stand alone rotator?
    //TODO: Can we nlerp or slerp motors
